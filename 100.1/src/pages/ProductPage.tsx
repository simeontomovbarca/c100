...
import { supabase, Collection, FAQItem, Review } from '../lib/supabase';
...
export function ProductPage() {
  ...
  const [availableNumbers, setAvailableNumbers] = useState<ShirtNumber[]>([]);
  ...
  const loadData = async () => {
    ...
    if (numbersData) setAvailableNumbers(numbersData);
  };

  const calculatePrice = (number: number | null): number => {
    if (!number) return 89.90;
  ...
  const handleBuyNow = () => {
    if (!selectedNumber) {
      alert('Please select a shirt number');
      return;
    }
    ...
    navigate(`/checkout?number=${selectedNumber}&size=${selectedSize}`);
  };
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <div className="text-amber-500 font-bold">{calculatePrice(selectedNumber).toFixed(2)} BGN</div>
              </div>
            </div>
            <button
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex items-baseline space-x-4 mb-6">
                  <span className="text-4xl font-bold text-zinc-100">{calculatePrice(selectedNumber).toFixed(2)} BGN</span>
                  {selectedNumber && calculatePrice(selectedNumber) > 89.90 && (
                    <span className="text-lg text-zinc-500 line-through">89.90 BGN</span>
                  )}
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="px-4 py-3 bg-zinc-900 border border-zinc-700 rounded-xl text-zinc-100 font-semibold text-center">
                  L - Large
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-zinc-900 border border-zinc-700 rounded-xl text-zinc-100 font-semibold focus:border-amber-500 focus:outline-none"
                >
                  <option value="">Choose your number...</option>
                  {availableNumbers.map((item) => {
                    const price = calculatePrice(item.number);
                    const isSpecial = price > 89.90;
                    return (
                      <option
                        key={item.number}
                        value={item.number}
                      ...
                      >
                        №{item.number} {isSpecial ? `- ${price.toFixed(2)} BGN` : ''} {item.is_sold ? '(SOLD)' : ''}
                      </option>
                    );
                  })}
                </select>
                {selectedNumber && calculatePrice(selectedNumber) > 89.90 && (
                  <div className="text-sm text-amber-500">
                    Special number pricing: +{(calculatePrice(selectedNumber) - 89.90).toFixed(2)} BGN
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                <button
                  onClick={handleBuyNow}
                  disabled={!selectedNumber || collection.is_sold_out}
                ...
                >
                  <ShoppingCart className="w-5 h-5" />
                  <span>Buy Now - {calculatePrice(selectedNumber).toFixed(2)} BGN</span>
                </button>
